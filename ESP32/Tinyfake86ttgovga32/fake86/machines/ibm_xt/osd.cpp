...
void Osd_t::OSDMenuRowsDisplayScroll(const char **ptrValue, unsigned char currentId, unsigned char aMax, uint32_t width, uint32_t pos, int32_t highlight)
{
...
uint8_t Osd_t::ShowTinyMenu(const char *cadTitle, const char **ptrValue, unsigned char aMax, uint32_t width, uint32_t pos, int32_t highlight)
{
  ...
  while (!bExit)
  {
    uint8_t scancode = machine->getKeyboard()->getLastKey();
    switch (scancode)
    {
...
void Osd_t::ShowTinyDSKMenu(uint32_t drive)
{
  ...
  if(list != nullptr)
  {
    ...
    if (selection != 0xFF)
    {
      ...
      reinterpret_cast<MachineXT_t *>(machine)->getDrive(drive)->openImage(fullpath);
      imgIndex[drive] = selection;
    }
  }
}

void Osd_t::ShowTinyVideoMenu()
{
...
void Osd_t::osdLeave()
{
  ...
  renderUpdateBorder();
  machine->getKeyboard()->Reset();
}

void Osd_t::svcDrawTableLoRes(uint32_t p)
{
...
uint8_t * Osd_t::svcGetPalette(uint32_t p)
{
  return graphPalettes[p];
}

void Osd_t::svcShowColorTable()
{
