...
void outVGA(unsigned short int portnum, unsigned char value)
{
  ...
  case 0x3D5: // cursor position latch
    ...
    else if (gb_portramTiny[fast_tiny_port_0x3D4] == 0xF)
    {
 ...
 */
    ...
    {
      if(value & 0x10) {
        setVideoParameters(VIDEO_MODE_GRAPH | VIDEO_MODE_80_COLS | VIDEO_MODE_COLOR, CGA_BASE_MEMORY);
      } else {
        setVideoParameters(VIDEO_MODE_GRAPH | VIDEO_MODE_40_COLS | VIDEO_MODE_COLOR, CGA_BASE_MEMORY);
      }
    } else {
      if (value & 0x01) {
        setVideoParameters(VIDEO_MODE_TEXT | VIDEO_MODE_80_COLS | VIDEO_MODE_COLOR, CGA_BASE_MEMORY);
