...
void memBrowser_t::repaint()
{
  ...
  for (uint32_t row = 0; row < MEM_ROW_COUNT; row++)
  {
    ...
    svcPrintText(buffer, area.left, area.top + row * ACTUAL_FONT_HEIGHT, FG_ADDR, BG_INACTIVE, 0);
    for(uint32_t col=0; col<MEM_COL_COUNT; col++)
    {
      ...
      svcPrintText(buffer, (NUMBERS_OFF + col * 3) * ACTUAL_FONT_WIDTH, area.top + row * ACTUAL_FONT_HEIGHT, FG_MEM_CONTENT, BG_INACTIVE, 0);
      position++;
    }
  }
}