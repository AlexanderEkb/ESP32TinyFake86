...
const uint32_t *ntsc_palette_even() { return atari_4_phase_ntsc_even; };
...
void __attribute__((optimize("-Ofast"))) IRAM_ATTR sync(uint16_t *line, int syncwidth) {
        // memset(line, SYNC_LEVEL, syncwidth *sizeof(uint16_t));
        for (int i = 0; i < syncwidth; i++)
                line[i] = SYNC_LEVEL;
}

void __attribute__((optimize("-Ofast"))) IRAM_ATTR blank(uint16_t *line, int syncwidth) {
    // uint8_t * base_sync  = (uint8_t *)line;
    // size_t length_sync   = (size_t)(syncwidth * sizeof(uint16_t));
    // memset(base_sync, SYNC_LEVEL, length_sync);

    // uint8_t *base_blank = base_sync + length_sync;
    // size_t length_blank = (size_t)((_line_width - syncwidth) * sizeof(uint16_t));
    // memset(base_blank, BLANKING_LEVEL, length_blank);
    ...
    uint16_t* buf = (uint16_t*)vbuf;
        ...
        if(!_pal_) {
            ...
            if (i < _active_lines) {                // active video
                ...
                if (i < (_active_lines + 5)) {   // post render/black
                    sync(buf, _hsync);
                    blank(buf, _hsync);
                    burst(buf);
